...
# Print the three-dimensional presentation
print("Three-dimensional presentation:", three_d_presentation)

# Create a PrettyTable instance
table = PrettyTable()
table.field_names = ["User ID", "Movie ID", "User Rating", "Is Favorite", "Genres", "Rating"]
...
for user_data in three_d_presentation:
    ...
    for movie_id, movie_info in movies.items():
        ...
        table.add_row([user_id, movie_id, user_rating, is_favorite, genres, rating])
...
def collaborative_filtering_recommendation(user_id, three_d_presentation):
    ...
    table = PrettyTable()
    table.field_names = ["Recommended Movie ID"]
    for movie_id in recommended_movies:
        table.add_row([movie_id])
...
# Test the collaborative filtering recommendation for User 7
# collaborative_filtering_recommendation(user_id, three_d_presentation)
...
def refinement_of_information(user_id, three_d_presentation):
    ...
    # Display the recommended movies in a table with similarity percentages
    table = PrettyTable()
    table.field_names = ["Top Similar Movie ID", "Similarity Percentage"]
    for movie_id, similarity_score in similar_movies[:5]:
        table.add_row([movie_id, f"{similarity_score * 100:.2f}%"])

    print(f"\nTop 5 Similar Movies for User {user_id} with Similarity Percentages:")
    print(table)
...
def merge_and_rank_recommendations(user_id, collaborative_recommendations, information_recommendations, all_movies, three_d_presentation):
    ...
    if not collaborative_movies and not information_movies:
    ...
    # Display the final recommendation list
    table = PrettyTable()
    table.field_names = ["Recommended Movie ID", "Similarity Percentage"]

    for recommendation in all_recommendations:
        if isinstance(recommendation, tuple):
            movie_id, similarity_percentage = recommendation
            table.add_row([f"{movie_id}", f"{similarity_percentage * 100:.2f}%"])
        else:
            table.add_row([f"{recommendation}", ""])

    print(f"\nFinal Movie Recommendations for User {user_id}:")
    print(table)

    return all_recommendations

#user_id = get_user_id_from_email('ali13797979@gmail.com')
merge_and_rank_recommendations(user_id, collaborative_filtering_recommendation, refinement_of_information, all_movies, three_d_presentation)
...
def recommended(email):
    ...
    for movie_id, similarity_percentage in all_recommendations:
